...
.tcg-deck-hits__card img{display:block;width:100%;aspect-ratio:245/342;border-radius:.125rem;object-fit:cover}
...
function DeckEditor({ deck, archetypes, onBack, onSaved, reload }: { deck?: Decklist; archetypes: Archetype[]; onBack: () => void; onSaved: (message: string) => void; reload: () => Promise<void> }) {
	...
	return <div className="space-y-5">
		...
		<section className={PANEL}>
			...
			{!!hits.length && <div className="tcg-deck-hits">{hits.map((hit) => <button key={hit.id} type="button" onClick={() => void addCard(hit)} className="tcg-deck-hits__card"><img src={hit.imageUrl} alt="" /><span>{hit.name}</span></button>)}</div>}
